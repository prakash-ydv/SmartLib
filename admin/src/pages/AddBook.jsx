...
const AddBook = ({ isOpen, onClose, onBookAdded, onBulkUploaded }) => {
  ...
  const transformRawDataToBackendFormat = (rawRows) => {
    ...
    rawRows.forEach((row) => {
      ...
      const key = row.isbn && row.isbn.toString().trim() !== ''
      ...
      const getFieldValue = (value) => {
        ...
        return (str === 'nan' || str === 'NaN' || str === 'null' || str === 'undefined') ? '' : str;
      };
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const rawDataStats = {
        ...
        validRawRows: rawData.filter(row => row.title && row.title.toString().trim() !== '').length,
        emptyRows: rawData.filter(row => !row.title || row.title.toString().trim() === '').length
      };
...
Digital Electronics,Fundamentals of Digital Design,Morris Mano,ECE,9788120333086,PHI,5th,https://example.com/digital.jpg,ACC004;ACC005;ACC006`;
  ...
  const handleDownloadErrorReport = () => {
    if (!uploadResult || !uploadResult.report || !uploadResult.report.errors) return;
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 mb-6 mt-10 border-2 border-blue-500">
      ...
      {isSubmitting && (
        <div className="mb-4 p-4 bg-blue-50 border-l-4 border-blue-500 rounded-lg">
          <div className="flex items-center">
            ...
            <p className="text-blue-800 font-medium">Adding book to library...</p>
          </div>
        </div>
      )}
        ...
        <div className="mb-6 p-6 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 rounded-xl shadow-md">
          ...
          {uploadResult.transformStats && (
            <div className="mb-4 bg-blue-50 rounded-lg p-4 border-l-4 border-blue-500">
              <h4 className="text-sm font-bold text-blue-800 mb-2">📊 Data Processing Summary</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                <div className="flex items-center space-x-2">
                  <span className="text-2xl">📥</span>
                  <div>
                    <p className="text-blue-600 font-semibold">Raw Entries</p>
                    <p className="text-xl font-bold text-blue-900">{uploadResult.transformStats.rawEntriesProcessed}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <span className="text-2xl">🔄</span>
                  <div>
                    <p className="text-orange-600 font-semibold">Grouped Duplicates</p>
                    <p className="text-xl font-bold text-orange-900">{uploadResult.transformStats.duplicatesGrouped}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <span className="text-2xl">📚</span>
                  <div>
                    ...
                    <p className="text-xl font-bold text-green-900">{uploadResult.transformStats.uniqueBooksCreated}</p>
                  </div>
                </div>
              </div>
              <div className="mt-3 p-2 bg-white rounded border border-blue-200">
                <p className="text-xs text-blue-700">
                  <strong>✅ {uploadResult.transformStats.rawEntriesProcessed} physical copies</strong> from your file were grouped into <strong>{uploadResult.transformStats.uniqueBooksCreated} unique books</strong> with merged accession numbers
                </p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="bg-white rounded-lg p-4 shadow-sm">
              <p className="text-xs text-gray-500 uppercase font-semibold">Books Processed</p>
              <p className="text-2xl font-bold text-gray-800">{uploadResult.report.totalRows}</p>
            </div>
            <div className="bg-green-100 rounded-lg p-4 shadow-sm">
              <p className="text-xs text-green-700 uppercase font-semibold">✅ Inserted</p>
              <p className="text-2xl font-bold text-green-800">{uploadResult.report.inserted}</p>
            </div>
            <div className="bg-blue-100 rounded-lg p-4 shadow-sm">
              <p className="text-xs text-blue-700 uppercase font-semibold">🔄 Updated</p>
              <p className="text-2xl font-bold text-blue-800">{uploadResult.report.updated}</p>
            </div>
            <div className="bg-yellow-100 rounded-lg p-4 shadow-sm">
              <p className="text-xs text-yellow-700 uppercase font-semibold">⚠️ Skipped</p>
              <p className="text-2xl font-bold text-yellow-800">{uploadResult.report.skipped}</p>
            </div>
          </div>
          ...
          <div className="mb-4">
            <div className="flex justify-between text-xs text-gray-600 mb-1">
              ...
              <span>{uploadResult.report.validRows} / {uploadResult.report.totalRows} valid rows</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div
                className="bg-gradient-to-r from-green-500 to-emerald-500 h-3 rounded-full transition-all duration-500"
                style={{ width: `${(uploadResult.report.validRows / uploadResult.report.totalRows) * 100}%` }}
              ></div>
            </div>
          </div>

          {/* Error Details Toggle */}
          {uploadResult.report.errors && uploadResult.report.errors.length > 0 && (
            <div className="mt-4">
              <button
                onClick={() => setShowDetailedReport(!showDetailedReport)}
                className="flex items-center space-x-2 text-sm text-red-700 hover:text-red-800 font-medium"
              >
                <svg className={`w-4 h-4 transition-transform ${showDetailedReport ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
                <span>{uploadResult.report.errors.length} Error(s) Found - Click to View</span>
              </button>
          ...
          {uploadResult.report.errors.length === 0 && (
            <div className="mt-4 flex items-center space-x-2 text-green-700">
              ...
              <p className="font-medium">All books uploaded successfully with no errors!</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="relative my-8">
        ...
        <div className="relative flex justify-center">
          <span className="px-4 bg-white text-gray-500 font-bold text-lg">OR UPLOAD BULK</span>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-xl p-6 border-2 border-dashed border-blue-300 shadow-inner">
        ...
        <div className="mb-6 bg-blue-100 rounded-lg p-4 border-l-4 border-blue-500">
          ...
          <ul className="text-sm text-blue-800 space-y-1 ml-7">
            <li>✅ Upload your college's raw data (one row per physical copy)</li>
            <li>🔄 System will automatically merge duplicate books</li>
            <li>📚 Accession numbers will be combined with semicolons</li>
            <li>🏛️ If department is missing in file, selected department will be used</li>
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-500 mt-1">
            💡 This will be used only for books that don't have a department in the Excel file
          </p>
        </div>
        ...
        {isUploading && (
          <div className="mb-4">
            ...
            <p className="text-xs text-gray-500 mt-1 text-center">Transforming data and uploading...</p>
          </div>
        )}
